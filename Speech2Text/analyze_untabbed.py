...
def requestCompleted(response, error, **context):
  ...
  if error is not None:
    ...
    if app == HODApps.RECOGNIZE_SPEECH:
      ...
      results['sentiment'] = aggregate['sentiment']
...
if hodApp == HODApps.RECOGNIZE_SPEECH:
  # paramArr["url"] = '/Users/natelevine/DevWeek16/Hackathon/Gauger/R0010025.MP4'
...
# response = hodClient.post_request(paramArr, hodApp, async=True, callback=asyncRequestCompleted, **context)
response = hodClient.post_request(paramArr, hodApp, False)
# response = hodClient.get_job_status(response['jobID'])
print response
# print results['score']
